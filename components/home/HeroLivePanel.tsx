...
export default function HeroLivePanel({
  ...
  return (
    <div className="glass hero-pane rise-in flex flex-col justify-between gap-5 rounded-panel p-6 sm:p-7">
      {/* Live header */}
      <div className="flex items-center justify-between">
        <p className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.16em] text-ink/60">
          <span className="pulse-dot h-1.5 w-1.5 rounded-chip bg-pine-bright" />
          {t("hero_liveNow")}
        </p>
      ...
      <Link
        className="group -mx-2 rounded-field px-2 py-1 transition-colors hover:bg-ink/[0.04] focus-visible:ring-2 focus-visible:ring-ink/20"
        href="/seasonal"
      >
        ...
        <p className="mt-2 flex flex-wrap gap-1.5">
          {inSeason.map((key) => (
            <span
              className="inline-flex items-center gap-1 rounded-chip bg-cloud/80 px-2.5 py-1 text-xs font-semibold text-ink/80 ring-1 ring-inset ring-line"
              key={key}
            >
          ...
          {extraCount > 0 ? (
            <span className="inline-flex items-center rounded-chip bg-pine/10 px-2.5 py-1 text-xs font-bold text-pine">
              +{extraCount}
            </span>
          ) : null}
        </p>
        ...
        <div>
          ...
          <ul className="mt-2 space-y-1">
            {newest.map((farm) => (
              <li key={farm.id}>
                <button
                  className="group flex w-full items-center gap-2 rounded-field px-2 py-1.5 text-left transition-colors hover:bg-ink/[0.04] focus-visible:ring-2 focus-visible:ring-ink/20"
                ...
                >
                  ...
                  <span className="shrink-0 rounded-chip bg-tone px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.08em] text-ink/60">
                    {farm.canton}
                  </span>
